...
class MegaDropDown extends React.Component {
    state = {name: '', categories: [], active: false, image: null, inside: false, prefix2: '/', prefix3: '/'}
    ...
    changeSubCategories = (categories, id, index) => {
        if (id==='1') {
            ...
            this.setState({image: categories.image, prefix2: '/' + categories.value + '/', prefix3: '/' + categories.value + '/'})
        } else if (id==='2') {
        ...
        return (
            <div
                ...
                <div className={this.state.active || this.state.inside ? 'mega-drop-down__content mega-drop-down__content--active' : 'mega-drop-down__content mega-drop-down__content--hide'}>
                    <ItemsColumn onMouseOver={this.inside} onMouseLeave={this.outside} id='1' func={this.changeSubCategories} urlPrefix='/'
